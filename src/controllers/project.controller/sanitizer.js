...
const updateProjectSanitizer = [
  body('id', 'Invalid Project ID').exists().isInt(),
  oneOf([
    body('description')
      .optional()
      .isString()
      .isLength({ max: 255 })
      .withMessage('Description must be a string with a maximum length of 255 characters'),
    body('title')
      .optional()
      .isString()
      .isLength({ max: 100 })
      .withMessage('Title must be a string with a maximum length of 100 characters'),
...
const likeProjectSanitizer = [
  cognito_sub,
  query('project_id', 'Invalid Project ID').exists().toInt(),
];
